...
public class StreamMethods {
	public static void main(String[] args) {
		 ...
		 * 3) Calulating 계열 : min, max, sum, average 등 
		 * 		 -> 산술연산관련된 최종 처리 메서드들 (기본자료형스트림에만 존재)
		 * 4) Matching : anyMatch, allMatch, noneMatch... 
		 * 		 -> 스트림 내부 요소에서 특정 조건을 만족하는 요소를 선택하기 위해 사용 
		 * 5) Reduction 계열 : reduce메서드를 통해 요소 내부의 값을 다양한 방법으로 연산 후 결과값을 반환하는 메서드 Calculating 계열에서
		 * 							  제공하지 않는 연산기능을 수행할 때 사용한다
		 */

		// 중간처리 메서드 (distinct, filter, forEach)
		List<Integer> list = Arrays.asList(2, 5, 5, 1, 6, 7, 7, 3);
		...
		// 4) Collect 계열 메서드
		// 4-1) Collectors.toList() : 스트림의 List를 메서드로 변환
		...
		// 5) Calculating 계열
			...
			// 5-3) summaryStatistics : 스트림 요소에 통계값을 가진 객체를 반환해주는 메서드
			// 		통계값 -> 총 개수, 합, 평균, 최소, 최대
		...
		// reduce (초기값, 초기값을 처리하는 람다식)
			...
			// Member 객체배열 arr에서 모든 회원의 나이의 합 구하기
			int ageSum = 
					arr.stream()
