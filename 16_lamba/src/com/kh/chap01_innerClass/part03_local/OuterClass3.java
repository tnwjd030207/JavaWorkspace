 ...
 * 
 * 	2) static키워드가 붙은 변수는 static메모리 영역에 저장공간이 할당된다
 * 		static final... -> static 메모리 내부의 constant pool에 할당 final 키워드만 붙은 변수도 constant pool에 저장공간이 임시로 할당된다
 */
	...
	public Runnable getRunnable(int num2) {
		final int local = 1; // 지역변수
		// local = 5; 수정하는 코드
		
		// 지연변수는 지역내부를 클래스에서 사용되는 경우, final예약어가 붙은 것처럼 취급한다
		// why? stack메모리에서 지역변수의 저장공간이 소멸된 후, 참조를 유지하기 위해서
		class LocalInnerClass implements Runnable{
			...
			@Override
			public void run() {
				//지역변수는 메서드 종료시 스텍공간에서 함께 소멸된다
				System.out.println(local);
				System.out.println(num);
				System.out.println(num2);
				System.out.println(str); //동일한 시점에 생성
				System.out.println(staticStr); //스테틱은 스테틱끼리 접근 가능하다
