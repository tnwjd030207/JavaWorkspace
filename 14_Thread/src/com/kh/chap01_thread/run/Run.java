...
public static void main(String[] args) {
	 ...
	 * 
	 * 멀티스레드
	 * 	- 메인스레드와 추가스레드를 이용하여 동시에 여러 작업을 처리한다
	 * 
	 * 멀티스레드의 장점
	 * 	1) A스레드, B스레드 나눠서 작업을 처리하므로 훨씬더 빨리 작업완료가 가능
	 *
	 *	2) 응답성 향상
	...
	for(int i=1; i<=100; i++) {
		...
		th1.start();
		//일반 스레도 생성 -> stack메모리 할당 -> main()메서드 실행
		//일반 스레도 생성 -> stack메모리 할당 -> run()메서드 실행
		 ...
		 * 
		 * 3. 독립성
		 * 	- 각 스레드는 독립성으로 실행되며, 다른 스레드의 영향을 받지 않는다
		...
		[소문자쓰레드 종료]
	*/
				Thread upperAlpThred = new Thread (()->{
