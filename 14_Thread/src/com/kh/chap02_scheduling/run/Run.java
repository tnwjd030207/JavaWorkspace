...
public class Run {
	public static void main(String[] args) {
		 ...
		 * 	- 만약, 스레드간에 우선순위가 부여된 경우 우선순위가 높은 스레드가 더 많은 작업시간을
		 * 	   할당받는다 (우선순위 기반 스케쥴링)
		 ...
		 * 우선순위 기반 스케쥴링
		 * 	- 스레드마다 우선순위를 부여하는 방식
		 * 	- 우선순위가 높은 스레드가 더 많은 작업시간을 할당받는데
		...
		Thread t3 = new Thread (()->{
			System.out.println(Thread.currentThread().getName());
			System.out.println("Tnak shooting...");
		 ...
		 * 	- 1~10 사이의 정수값을 대입하여 우선순위를 변경할 수 있다
		 */
	//      중요도
		...
		t1.setDaemon(true);
		t2.setDaemon(true);
		t3.setDaemon(true); //모든 스레드를 데몬스레드로 설정해야, 설정 완료
