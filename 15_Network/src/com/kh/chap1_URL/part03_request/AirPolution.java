package com.kh.chap1_URL.part03_request;
...
public class AirPolution {
	public static void main(String[] args) throws UnsupportedEncodingException {
		 ...
		 * 			  애플리케이션들 중 데이터나 서비스를 제공하는 역할을 API서버라고 부른다
		 * 			  (카카오  API서버, 네이버 API서버 등)
		 * 			- A애플리케이션이 B애플리케이션에 필요한 데이터가 있는 경우,
		 * 			   B애플리케이션의 API서버를 통해 요청(reqeust)을 보내고, B API서버는 요청에 대한 응답(response)을 돌려준다
		 * 			- 단, API서버 요청시에는 해당 API서버가 정한 규칙(interface)에 맞춰 요청을 보내야 한다
		 * 
		 * 		2. API 서버 요청 방식 (Http Method)
		 * 		- API서버에서 저장한 규칙에 따라야 하며, 전통적인 방식의 API요청 방법은 2가지가 있다
		 * 			1) Get방식
		 * 				- URL애 요청변수(queryParameter)를 붙여서 요청하는 방식
		 ...
		 * 			3) 처리방법
		 * 				- xml, json 등 해당 타입에 맞는 파서를 이용하여 내 프로그래밍언어에 맞는 개체로 변환한다
		 * 		
		 * 		4. API 응답 상태코드
		 * 		- 200 성공
		 * 		- 404 : 찾고자하는 자원이 없는 경우 전달하는 상태값 (오류)
		 * 		- 400 : 요청 오류
		 * 		- 401 : 인증 오류시 전달하는 상태값
		 * 		- 500 : API서버상의 오류가 발생한 경우 전달하는 상태값
		 */
		String serviceKey = "5bf7cc651922051b154ef0691d022324c10abf5232b019bdcbf7b603301f0b39";
