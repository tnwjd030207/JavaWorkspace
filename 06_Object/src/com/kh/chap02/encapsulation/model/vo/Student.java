...
public class Student {
	...
	// setter / (*필수)getter 메서드
	
	//name필드에 값을 초기화(대입)하는 용도의 메서드
	//초기화를 위해선 값을 얻어와야 하므로 매개변수가 존재한다
	//초기화를 진행한 후 값을 반환할 필요는 없기 때문에 반환형은 void
	public void setName(String name) {
		// 매개변수 : 메서드 호줄 시 전달되는 값을 받아주기 위한 변수 선언 방법
		this.name = name;
		//name = name; <- 필드명과 매개변수명이 동일한 경우, {}내에서 선언한 변수의 우선순위가 높기 때문에
		//				  name은 항상 매개변수 name을 가리키게 된다
		// 필드로서의 name을 가리키고자 한다면 this.name을 사용할 것!
		
		//*this
		//- 모든 매서드의 묵시적으로 존재하는 변수로, 생성된 객체의 주소값이 담겨있다
