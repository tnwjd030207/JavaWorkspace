...
public class Run {
	public static void main(String[] args) {
		...
		g1.setT(1111);
		g1.setG("1111");
		Generic.printTest("머든"); //제네릭 미지정시 Object
		Generic.printTest("머든"); //제네릭 미지정시 Object
		//Generic.<String>printTest(111111111);
		...
		/*
		 * 3. 제네릭의 extends 문법
		 * 	- 제네릭에서의 extends는 클래스간의 상속 개념이 아닌, 제네릭으로 지정한 자료형의
		 * 	  범위를 "제한"시키는데 사용한다
		 * 	- <T extends Parent> : T(타입변수)에는 Parent클래서나 Parent클래스를 상속 받은 자료형만 대입 가능
	...
	//	Parent 및 Parent 자식이 아니기 때문에 타입 설정 오류
	//	GenericExtends<String> str = new GenericExtends<>();
		...
		//	?의 제대로된 사용을 위해선 extends와 super을 사용하여 ?의 범위를 제한해야 한다
		
		
		//	1) extends를 통한 와일드카드 상한제한
		GenericExtends<? extends Parent>unkown2 = new GenericExtends<>();
...
//		?의 범위에 Parent의 자식 Child1,Child2가 포함될 수 있으므로 대입불가능한 상황이 생긴다
//		unkown2.setType(new Child1());
//		? 범위에 Child2도 포함되어 있으므로 대입불가능한 상황이 생긴다		
//		unkown2.setType(new Child2());
		
		//unkown2가 Child1이든 Child2든, 다형성에 의해 값을 추출하려 대입 가능
		...
		// 값을 "대입"하고자 할 때는 super을 통한 하한제한을 사용하는 것이 적절하다
		
		Parent p3 = unknown3.getType(); // ?는 Object일 수 있음 단, 클래스레벨 제네릭 설정시 
		// T extends Parent 로 설정했기 때문에 T의 범위는 Parent~Parent의 자식클래스로 한정
