package co.kh.chap04_map.part01_hashMap.run;

import java.security.KeyStore.Entry;
import java.util.HashMap;
import java.util.Set;

import co.kh.chap04_map.part01_hashMap.model.vo.Snack;
...
public static void main(String[] args) {
		 ...
		 * 		   사용하는 메서드가 다르다
		 * 		- 데이터를 저장할 때 key, value를 한 쌍으로 저장하고,
		 * 		   key를 통해 value가 저장된 위치에 접근한다
		 * 		- key값의 중복을 허용하지 않는다
		...
		hm.put("다이제", new Snack("초코맛", 900)); //동일한 값은 덮어버린다
		...
		//		- keySet() : 맵의 key값들을 따로 Set에 담아 변환해주는 메서드
		...
		//		- 맵의 key, value값이 담긴 entry를 set에 담아 변환해주는 메서드
		Set<java.util.Map.Entry<String, Snack>> entrySet = hm.entrySet();
		for(java.util.Map.Entry<String, Snack> entry : entrySet) {
